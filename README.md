...
| 笔记 | 内容 |
|---|---|
| [LSM Tree 原理：从零造一个 KV 存储](storage/LSM树原理.md) | 不讲理论，从最笨的「只往文件末尾追加」开始，十步推导出完整的 LSM：稀疏索引 → SSTable → MemTable（含跳表原理插曲）→ WAL → Compaction → 分层 → 布隆过滤器 → 墓碑，每一步都被上一步的问题逼出来，最后对应到 RocksDB 的实际名词和参数 |
