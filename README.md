...
| 笔记 | 层次 | 内容 |
|---|---|---|
| [B+ 树原理](mysql/B+树原理.md) | **为什么** | 为什么不用哈希/红黑树/B 树、用具体数据把树完整拆开（上下层 key 如何对应、叶子页里一行怎么存）、3 层为什么能装 2190 万行、页内 Page Directory 二分、页分裂与自增主键、聚簇索引 vs 二级索引、**联合索引专章**（元组排序、六条 SQL 的树上走位、IN 不截断、ORDER BY 免 filesort、列序三步法）、深分页为什么慢 |
